...
//! Prop types vendored from unmaintained `@astropub/icons` (dist/Props.ts).
//! Imported with `import type` only, so nothing is emitted at build time
//! (a value-style `export { Props }` re-export is what broke Vite 8 / Rolldown).
import type { HTMLAttributes as AstroHTMLAttributes } from "astro/types";
...
export interface HTMLAttributes extends AriaAttributes {
  ...
  lang?: string;
  placeholder?: string;
  role?: AstroHTMLAttributes<"svg">["role"];
  slot?: string;
